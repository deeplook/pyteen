...
def pip_installable(pkg_name: str, index_url: str = "") -> bool:
    ...
    The default value for ``index_url`` is "https://pypi.org/pypi".
    """
...
def validate(source_code: str, raise_immediately: bool = True) -> str:
    ...
    num_mt = len([tok for tok in toks if tok.type == token.NL and tok.string == "\n"])
    num_lines = len(source_code.splitlines())
    num_nl = len(
        [
            tok
            for tok in toks
            if tok.type == token.NEWLINE and tok.string == "\n" and tok.line != ""
        ]
    )
